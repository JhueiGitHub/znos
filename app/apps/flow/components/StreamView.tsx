  ...
  const { mutate: unpublishFromXP } = useMutation({
    mutationFn: async (flowId: string) => {
      const response = await axios.delete(
        `/api/xp/publications/by-flow/${flowId}`
      );
        ...
        if (publishedFlowsMap[flow.id]) {
          ...
          if (initialContent) {
            const hasContentChanged =
              JSON.stringify(flow.components) !==
              JSON.stringify(initialContent.components);
                    ...
                    <>
                      ...
                      <ContextMenuItem
                        onClick={() => {
                          if (
                            window.confirm(
                              "Are you sure you want to unpublish this flow from XP?"
                            )
                          ) {
