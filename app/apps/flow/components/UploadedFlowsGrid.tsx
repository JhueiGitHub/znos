...
export const UploadedFlowsGrid = ({ profile }: UploadedFlowsGridProps) => {
  ...
  if (isLoading) {
    return (
      <div className="text-sm" style={{ color: getColor("Text Secondary (Bd)") }}>
  ...
  if (!publications?.length) {
    return (
      ...
      >
        ...
        <p className="text-sm mt-2">Right-click an Orion flow to publish it to XP</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-3 gap-6">
      {publications.map((pub: any, index: number) => (
        ...
        >
          <Card
            className="h-[200px] cursor-pointer hover:border-white/20 transition-colors"
            style={{
              backgroundColor: getColor("Glass"),
              borderColor: getColor("Brd"),
            }}
          >
            <CardContent className="p-6">
              ...
              <div className="mt-4 flex items-center gap-4">
                ...
                >
                  <span className="font-medium">{pub.downloads}</span> downloads
                </div>
                <div
                  className="text-sm"
                  style={{ color: getColor("Text Secondary (Bd)") }}
