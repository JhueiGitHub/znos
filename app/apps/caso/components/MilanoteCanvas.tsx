...
const MilanoteCanvas: React.FC = () => {
  ...
  const handleDoubleClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      // Add a completely blank note with no title or text
      // and store the position in local storage
      const noteId = addItem(
        currentBoardId,
        "note",
        { x, y },
        {
  ...
  // 3. Replace the handlePaste function to create notes with URL content
  const handlePaste = useCallback(
    (e: ClipboardEvent) => {
      ...
      // Prevent duplicate paste handling
      ...
      // Create a note with just the pasted content, no title
      // and store the position in localStorage
      const noteId = addItem(
        currentBoardId,
        "note",
        { x, y },
      ...
      >
        {[...items]
          ...
          .map((item) => {
            switch (item.type) {
              case "note":
              case "link": // Handle both note and link types with our unified component
                return (
                  <UnifiedNote
                    key={item.id}
                    id={item.id}
