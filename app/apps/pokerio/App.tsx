...
import { Socket, io } from "./classes/socket.io";
import AsyncTask from "./components/AsyncTask";
import { TranslateCode } from "./classes/code";
import "./pokerio.css";
      ...
      <div className="gui">
        ...
        <div className="player-list">
          {playerList.map((player) => (
            <div className="player-info" key={player.name}>
              ...
              <div className="actions" data-has={actions.slider != undefined}>
                {Object.entries(actions.functions).map(([value, func]) => (
                  <button
                    key={value}
                    id={value}
                    disabled={typeof func === "object" && func.disabled()}
                    onClick={() => {
        ...
        <div className="information">
          <img src="/apps/pokerio/donald.png" alt="" />
          <div className="inner">
            <p>{name}</p>
            <hr />
...
export default function PokerApp(args: {
  name: string;
  ip?: string;
  create?: boolean;
}) {
    ...
    <AsyncTask<connectionResultType>
      ...
      meanwhile={
        <article className="progress">
          <main>
            <img src="/apps/pokerio/iconclean.png" />
          </main>
        </article>
      }
      reject={(r) => (
      ...
      resolve={(sock) => (
        <AssetLoader
          assets={urls}
          resolve={(data) => <App data={data} name={args.name} socket={sock} />}
        />
      )}
    />
  );
